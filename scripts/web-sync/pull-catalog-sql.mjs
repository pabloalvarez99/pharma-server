#!/usr/bin/env node
// pull-catalog-sql.mjs — Patrón A (Pull) de ADR-0012 (seam Tu Farmacia).
// Renombrado desde pull-catalog.mjs: ese nombre ahora es el cliente del seam
// Free Web (`/api/v1/public/{slug}/catalog`, ADR-0020).
//
...
//   node scripts/web-sync/pull-catalog-sql.mjs
//
// Exit codes:
//   0  OK, SQL generado.
...
export function buildSql(items, valueRows, tenant, generatedAt) {
  const header = [
    `-- catalog_upsert.sql`,
    `-- Generated by scripts/web-sync/pull-catalog-sql.mjs at ${new Date().toISOString()}`,
