//! «¿cuánto hay en caja?» en feria habla de puesto, no de cajón de farmacia.
...
async fn feria_sin_sesion_habla_de_puesto() {
    ...
    assert_eq!(a.data.as_ref().unwrap()["open"], false);
...
async fn feria_con_sesion_habla_de_puesto() {
    ...
    let a = ask(&db, &tenant, "cuánto hay en caja").await;
    assert_eq!(a.intent, "caja_actual");
    assert_eq!(a.data.as_ref().unwrap()["open"], true);
    let t = a.text.to_lowercase();
    assert!(t.contains("puesto"), "text: {}", a.text);
    assert!(
        !t.contains("en la caja"),
        "no debe citar la caja de farmacia: {}",
        a.text
    );
}
...
async fn sin_vertical_sigue_caja_de_farmacia() {
    ...
    assert_eq!(a.data.as_ref().unwrap()["open"], false);
    assert_eq!(
        a.text, "No hay ninguna caja abierta en este momento.",
