...
8. **Secrets**: nunca commitear `config/local.toml` ni `data/`. JWT secret de `config/default.toml` (`change-me-in-production`) es placeholder; producción inyecta vía env `PHARMA__JWT__SECRET`. Loader: `config/default.toml` → `config/local.toml` (opcional) → env `PHARMA__*` separator `__`.
   ...
   **El guard atrapa una sola cadena, y no es la única que existe.** `check_jwt_secret` (`crates/api/src/lib.rs`) compara contra la constante `PLACEHOLDER_JWT_SECRET = "change-me-in-production"` y aborta el arranque. Pero `default_config()` en el mismo archivo construye el fallback con `"change-me"` — **otra cadena, que pasa el guard**. El test tampoco lo ve: sólo ejercita el literal que el guard ya conoce, así que comparte exactamente el punto ciego del bug. Cualquier arranque que caiga en el fallback de código en vez de leer el `.toml` firma tokens con un secreto público. **Al tocar el placeholder hay que cambiar los tres lugares**, y el test tiene que probar el camino de `default_config()`, no la constante.

   **Credenciales de terceros nunca van en `config/*.toml` versionado.** El bloque `[user_backup]` es el patrón: `access_key_id = ""` / `secret_access_key = ""` vacíos en el default, valor sólo por entorno (`PHARMA__USER_BACKUP__ACCESS_KEY_ID`, `PHARMA__USER_BACKUP__SECRET_ACCESS_KEY`). Lo mismo aplica al client secret de Google si algún día hace falta uno de servidor — el flujo de Android usa client IDs, que son públicos por diseño y no son secretos.
...
11. **Nube: no se compra infraestructura antes de que un número la exija (capitán, 2026-08-10)**. El proyecto corre a costo cero y sin tarjeta. Un servicio pago entra sólo cuando hay una medición que muestre que no entrar cuesta más caro, y esa medición se archiva junto a la decisión.

    **Caso que fijó la regla — respaldo de usuario.** ADR-0023 recomendaba comprar R2. Se midió: un sobre cifrado pesa 65,8 KB, cinco versiones por negocio ~330 KB, mil negocios 330 MB. SurrealKV embebido come eso sin despeinarse. Conclusión: **el sobre va en la base**, detrás del mismo `BlobStore` (`crates/api/src/v1/user_backup/store.rs`) — un trait de cuatro métodos con backends elegidos por un string de config. El backend `s3` queda escrito y testeado, sin usar; el día que haga falta se cambia el string. Los números de ADR-0023 siguen valiendo, cambia la conclusión: dejó de ser "elegimos R2" y pasó a ser "medimos, y a esta escala no hace falta comprar nada".

    **Disparador para revisar**: el día que perder el server sea inaceptable. Es el mismo día en que hacen falta respaldos de servidor de verdad, y no es hoy con cero usuarios y cero APKs publicadas. La objeción honesta —el respaldo vive en la base que respalda— no aplica al caso de uso real, que es *"perdí el teléfono"*: el sobre sigue estando fuera del aparato, que es la propiedad que importa ahí.

    **Lo que sí se descubrió midiendo y hay que recordar**: a este tamaño de objeto la factura de object storage **son los PUT, no los bytes** (96% Class A). Por eso el límite que controla costo es `max_uploads_per_day`, no el tamaño ni los `ListObjects`. Y si algún día se compra GCS: el nivel gratuito existe sólo en `us-west1` / `us-central1` / `us-east1` — `southamerica-west1` cobra desde el primer byte, y la latencia da igual para un blob de 66 KB que se baja una vez en la vida.

    **Google Cloud para login sí, y es gratis**: proyecto + pantalla de consentimiento + client IDs de Android no piden facturación. Dos trampas que cuestan una tarde cada una: (a) la pantalla de consentimiento en modo **Testing** sólo deja entrar a las cuentas listadas a mano — hay que publicarla a **Production**; (b) hacen falta **dos** client IDs de Android, con la huella SHA-1 de debug **y** la de release — el APK firmado con la otra llave falla con un error que no dice cuál de las dos es. Detalle completo y estado de la cuenta en `work/decisions/2026-08-10-respaldo-sin-bucket-y-cuenta-google.md` del vault.
