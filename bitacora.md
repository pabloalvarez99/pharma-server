...
- **🎯 VERDAD PRESENTE (2026-07-26) — MULTI-SUCURSAL COMPLETO DE PUNTA A PUNTA (V2 → V2.1 FEFO → vencimientos)**: `feature/saas-web` tiene las tres capas integradas y gateadas. **(1) V2 (migración 0041)**: el stock dejó de ser un contador global y pasó a ser **por local** — `product_branch_stock` mantenida por evento sobre `stock_movement`, venta branch-aware (request → caja abierta → casa matriz), transferencias A→B de suma cero, `tenant_stock_lock` compartido por venta/devolución/transferencia. **(2) V2.1 FEFO (migración 0042)**: el **lote** tiene domicilio (`product_batch.branch`), el FEFO de una venta sólo consume lotes de SU local, la recepción entra a la sucursal fijada en la OC y la transferencia mueve lote-por-lote (el destino puede vender lo que recibe). **(3) Vencimientos por sucursal (lane `feat/vencimiento-por-sucursal`, sin migración)**: el digest nocturno de `jobs::near_expiry` agrupa por **(tenant, local)** y el reporte `GET /api/v1/reports/near-expiry?branch=` filtra por local devolviendo `branch`+`branch_name`; el cliente tiene filtro de local en "Próximos a vencer", columna Local en los lotes, alta de lote en la sucursal activa, selector "Entra a" al crear la OC y local read-only en la recepción; el agente nombra el local en cada lote por vencer. **Invariantes vivos**: `Σ product_branch_stock == product.stock == Σ stock_movement.delta` (0041) y `Σ product_batch.stock WHERE branch = X == product_branch_stock[X]` (0042). **Vocabulario único de sucursal** en toda la API: ausente = todos los locales · `"none"` = casa matriz · `branch:<key>` = ese local. **Migración libre = 0043.**

- **📌 PENDIENTE — lo que multi-sucursal NO cubre todavía**: (a) **`agent_orders` despacha siempre desde casa matriz** (su `stock_movement` no lleva `branch`) — elegir local por voz es lane aparte; (b) **alertas de vencimiento por canal real** (Telegram/email): hoy el job sólo emite el digest a `tracing::warn!`, nadie lo recibe fuera del log; (c) **dashboard de vencimientos queda GLOBAL a propósito** (foto del negocio entero, con el local en cada fila) — si el fundador lo quiere atado al selector "Estás en &lt;local&gt;" del topbar, es un parámetro, no un rediseño; (d) **móvil**: `tauri android init` está commiteado y el updater quedó desktop-only, pero **falta el build real de APK en dispositivo** y la pasada de UX táctil.

- **🧪 LECCIÓN DE GATE (2026-07-26, costó un rojo en `feature/saas-web`)**: la lane V2 gateó **sólo con cargo** y metió `sucursales` al gate de `physicalStock` en `first-run.ts` sin actualizar `client/e2e/rubro-gating-matrix.test.ts` → `npm run test` quedó **rojo** (belleza y servicios) desde el merge de V2 hasta que lo arregló la lane de vencimientos. **Regla**: toda lane que toque `client/src` corre `npm run build` + `npm run test`, no sólo el GATE de cargo. El gate de Rust no ve el cliente.

- **✅ 2026-07-25 — V2 MULTI-SUCURSAL OPERATIVO: stock por local + transferencias (lane `feat/stock-multisuc-v2`)**: hasta hoy `branch`/`register` (migración 0032) eran **sólo records de configuración** — tener 2 locales no cambiaba nada: el stock era un único contador global por producto. Ahora el stock es POR SUCURSAL de punta a punta. **Migración 0041**: `stock_movement.branch` · tabla **`product_branch_stock`** (on-hand por `(tenant, producto, sucursal)`, id determinístico `[tenant, product, branch|'none']`) mantenida incrementalmente por `DEFINE EVENT product_branch_stock_maint` sobre `stock_movement` CREATE (mismo patrón probado que `product_stats_maint` 0029; el ledger es append-only ⇒ evento CREATE puro) · `order.branch` (venta atribuida a su local) · `cash_register_session.register`/`.branch` (la caja se abre EN un local) · backfill del stock histórico a **casa matriz** (bucket `NONE` = negocio de un solo local, comportamiento idéntico a pre-V2). **Invariante V2** (testeado): `Σ product_branch_stock == product.stock == Σ stock_movement.delta`. **Venta branch-aware**: `PosSaleRequest.branch` explícito → si no viene, la sucursal de la **caja abierta del cajero** → si no hay, casa matriz; el pre-chequeo de stock ahora corre contra el bucket del local (acumulando líneas repetidas del mismo SKU), así que **no se puede vender en A lo que está en B aunque el stock global alcance**. Devolución repone en la sucursal de la venta original. **Transferencia A→B** = dos `stock_movement` de **suma cero** (`transfer_out` −qty / `transfer_in` +qty) en UNA transacción: mueve la distribución, nunca el total; rechaza sobre-giro del origen, mismo-local, sucursal de otro tenant y servicios (`physical_stock=false`). **Concurrencia (BUG-003/004 NO se reabre)**: el `SALE_LOCKS` por tenant se hoisteó a `crate::locks::tenant_stock_lock` y ahora lo comparten venta, devolución **y transferencia** — un lock por flujo NO habría serializado venta-contra-transferencia sobre las mismas filas (`product` / `stock_movement` / `product_branch_stock`). **2 agujeros preexistentes cerrados de paso**: (a) el retiro de pedido web bajaba `product.stock` **sin emitir `stock_movement`** (rompía el invariante raíz de auditoría) → ahora emite `reason='web_pickup'` con la sucursal de la orden; (b) `catalog::create_product` escribe el stock inicial directo (único punto donde el stock nace sin ledger) → evento `product_branch_stock_seed` sobre `product` CREATE lo siembra en casa matriz, si no un producto nuevo con stock quedaba invendible en todo local. **API** (`crates/api/v1/stock.rs`): `GET /api/v1/stock/sucursales` + `GET /api/v1/stock/sucursales/reporte` (cajero+) · `POST /api/v1/stock/transferencias` (admin+). **Cliente completo**: selector **"Estás en &lt;local&gt;"** en el topbar (persistido por terminal en `localStorage`, oculto si el negocio tiene un solo local, re-renderiza la vista al cambiar) · vista **Sucursales** (tabla pivote producto × local + total, columna del local activo resaltada, scroll horizontal dentro de la tarjeta para móvil) · **modal de transferencia** (muestra el disponible real del origen antes de mover, valida contra él, y traduce `INSUFFICIENT_STOCK` a "alguien vendió o movió stock recién") · POS manda la sucursal activa · apertura de caja la hereda. 5 comandos Tauri nuevos + sus handlers del shim web (funciona igual en PWA). **Tests**: `crates/domain/tests/stock_branches.rs` — aislamiento (venta en A no toca B; no vender en A el stock de B; líneas repetidas no burlan el saldo), conservación del total en transferencia, no-sobre-girar, auditoría de suma cero, herencia desde la caja, devolución al local correcto, reporte cuadrado con el stock global, aislamiento multi-tenant, y **dos de concurrencia** (20 ventas paralelas en el mismo local; ventas ∥ transferencias sobre el mismo producto). **Migración libre = 0042.**

- **✅ 2026-07-25 — V2 INTEGRADA: GATE verde + SMOKE EN VIVO + merge a `feature/saas-web`**: las 3 lanes del rescate quedaron mergeadas `--no-ff` y pusheadas — `feat/assist-abono` (`cb057a3`), `feat/mobile-init` (`c20b474`) y `feat/stock-multisuc-v2` (`c0a9aba`, con el fix `eae0dae`). **GATE V2**: `cargo fmt --check` + `clippy -p domain -p api --all-targets -D warnings` + `test -p domain` (21 suites) + `test -p api` (44 suites, **`e2e_concurrency_fefo` 4/4** — el guardián de BUG-003/004 sigue verde con el lock hoisteado). **🐛 Bug de producción cazado por el GATE**: `cash_register::service::open_session_branch` ordenaba `ORDER BY opened_at` sin seleccionar el campo → SurrealDB rechaza la query completa (`Missing order idiom \`opened_at\` in statement selection`), o sea **toda venta de un cajero con caja abierta fallaba**; fix = `SELECT branch, opened_at ...`. **Gotcha para la próxima query**: en SurrealDB el campo del `ORDER BY` DEBE ir en la proyección. **SMOKE EN VIVO** (server real + SurrealKv nuevo, 41 migraciones aplicadas, 2 sucursales): producto nuevo con 20 → casa matriz 20 (evento `product_branch_stock_seed`) → reparto 7 matriz / 8 A / 5 B → **venta de 3 en A**: A 8→5, **B intacto en 5**, matriz intacta en 7, global 17 → **transferencia A→B de 2**: 3/7, **total conservado 17 = `product.stock`** → sobre-giro de 999 rechazado con **422**. **Alcance real de V2 (para no sobrevenderlo)**: stock por sucursal = **cantidades** + transferencias + sucursal en venta/caja. **Lotes/FEFO por sucursal NO se hizo** — sigue siendo la lane V2.1 del pendiente de arriba.

- **✅ 2026-07-25 — Cliente móvil nativo, primer paso (lane `feat/mobile-init`)**: `tauri android init` commiteado (`client/src-tauri/gen/android`, ~16 MB de scaffolding versionado; sólo se ignoran los artefactos de build). `tauri-plugin-updater` pasa a **desktop-only** (el crate declara soporte android/ios "none"; en móvil el canal de update es la tienda): gateado por target en `Cargo.toml` y por `#[cfg(desktop)]` en `lib.rs`, con `updater:default` movido a `capabilities/desktop.json` (el `default.json` queda para toda plataforma). `bundle.android.minSdkVersion = 24` + scripts npm `android:init|dev|build` / `ios:init|dev`. GATE: `clippy --all-targets -D warnings` del crate cliente + `tsc --noEmit` + vite + vitest **748 verdes**. Falta (lane siguiente de móvil): build real de APK en dispositivo/emulador y la pasada de UX táctil.

- **✅ 2026-07-22 — El AGENTE aprende fiado + IVA (lane `feature/saas-web`)**: cierra el círculo del norte "1 RUT = 1 agente" — las capacidades nuevas (fiado, libro de compras/IVA) ahora se pueden PREGUNTAR, no solo mirar en una vista. Dos intents nuevos en `crates/assist`: **`PorCobrar`** ("¿cuánto me deben?", "quién me debe", "cuentas por cobrar", "deudores", "fiado") → total por cobrar + los 2 que más deben, sobre `credit::repo::debtors`; **`IvaMes`** ("iva", "iva del mes", "cuánto impuesto pago", "f29") → IVA de ventas vs compras y si toca pagar o queda a favor, sobre `compliance::repo::iva_summary`, **avisando cuándo la cifra es estimada** (facturas de compra sin capturar). Texto de ayuda del agente actualizado. **Gotcha del matcher**: `"iva"` es substring de palabras comunes (*activa*, *motiva*) → el patrón exige límite de palabra (`q == "iva"`, `starts_with("iva ")`, `" iva"`), con test dedicado que verifica que "clientes activa" NO clasifica como IVA. Cero cambios de cliente: `assist_ask` y el ask-bar ya existen, las respuestas nuevas fluyen solas (también en web). Tests assist 84 verdes (3 nuevos).

- **✅ 2026-07-22 — V3 LIBRO DE COMPRAS + RESUMEN IVA/F29 (backend, lane `feature/saas-web`)**: el ERP emitía libro de VENTAS (DTE) pero no el de COMPRAS — el dueño no podía sacar su IVA crédito ni armar el F29. **Migración 0040**: datos de la FACTURA del proveedor sobre la OC (`invoice_folio/date/tipo/neto/iva/total`, todos opcionales). **`crate::compliance`**: libro del período (fila por documento con proveedor+RUT), **derivando neto/IVA del total al 19%** mientras el operador no capture la factura real (fila `declared=false` + `pending_declaration` → el dueño ve qué le falta); resumen **IVA débito (ventas) − crédito (compras) = a pagar** + netos. **API admin+**: `GET /api/v1/reports/libro-compras?period=YYYY-MM`, `GET /api/v1/reports/iva?period=YYYY-MM` (default mes en curso), `PATCH /api/v1/purchase-orders/{id}/factura`. Tests 6/6. GATE fmt+clippy -D + domain 20 suites + api 44. **SMOKE EN VIVO**: OC → send → receive → libro derivado (neto 10000 / IVA 1900, `declared=false`) → captura factura `A-9912` → `declared=true` → IVA crédito 1900. **🐛 BUG DE PRODUCCIÓN encontrado y arreglado en el smoke**: `include_dir!` embebe `migrations/` en compile-time pero **cargo no rastrea el contenido del directorio** → agregar una migración NO recompilaba `db` y el binario salía con el set viejo; la migración nunca se aplicaba y la falla es **silenciosa** (tabla SCHEMAFULL descarta campos no definidos, el UPDATE responde 200 sin persistir). Fix: `crates/db/build.rs` con `rerun-if-changed` sobre `migrations/` y cada archivo (verificado: 39→40 migraciones aplicadas). Commits `9f30117` (backend) + `b0ca1af` (build.rs) + cliente. **Migración libre = 0041.** **Cliente V3 ✅**: panel "IVA del mes (F29)" + tarjeta "Libro de compras" en Reportes (filas sin factura marcadas "estimado" + nota de cuántas faltan), export CSV/JSON con columnas de contador, comandos Tauri + shim web; cliente 747 tests verdes.

- **✅ 2026-07-22 — V1 FIADO / cuenta corriente COMPLETO punta a punta (lane `feature/saas-web`)**: el #1 esencial del retail chico CL (`business-depth-master-plan.md` V1) — antes NO existía, el almacenero iba con cuaderno. **Ledger APPEND-ONLY inmutable** (migración **0039** `customer_ledger`): cada `cargo` (venta fiada) y `abono` (pago) es una fila; el saldo se CALCULA sumando, nunca se muta un total. `crate::credit` (model/repo/service): estado de cuenta (saldo + total fiado + total abonado + movimientos), `post_cargo` **idempotente por orden**, `record_abono` (valida monto>0, que haya deuda, y que no la supere). **POS**: nuevo tender `pos_fiado` — exige cliente, NO mueve caja, genera el cargo tras confirmar la venta; 0039 agrega `pos_fiado` al whitelist de `order.payment_method` (`DEFINE FIELD OVERWRITE`, append-only — el test cazó que sin esto la venta fiada NO persistía). **API cashier+**: `GET /api/v1/customers/{id}/cuenta`, `POST /api/v1/customers/{id}/abono` (`cash_session` opcional → el abono en efectivo entra al arqueo). **Cliente**: tender "Fiar" en POS (Cobrar bloqueado sin cliente) + bloque "Cuenta corriente" en la ficha de Clientes (saldo destacado, form de abono, tabla de movimientos; si el server no tiene el módulo el bloque no se muestra y la ficha no se rompe); comandos Tauri `customer_account`/`record_abono` + wrapper `api/credit.ts` + handlers del shim web (fiado funciona igual en browser). **GATE verde**: domain 51 suites (5 tests nuevos de fiado) + api 44 + fmt/clippy -D + cliente tsc/vitest **744** (3 shim nuevos) + build desktop + build:web + clippy del crate Tauri. **SMOKE EN VIVO** (server local 8091, DB limpia, migraciones al boot): fiar 5580 → saldo 5580 → abono 1000 → saldo 4580 → `abono>deuda` rechazado ("el abono (999999) supera la deuda pendiente (4580)") → `fiar sin cliente` rechazado ("para fiar debes elegir el cliente"). Commits `ec59fa8` (backend) + `53257c2` (cliente). **Migración libre = 0040.**

- **✅ 2026-07-21 — SP4 SaaS web: signup web gratis (repo hermano `pharma-license-server`, PR #4)**: cierra el pack SaaS web (SP1→SP4). Flujo "Usar gratis en web": `/signup` (form: nombre, RUT mód-11 con hint en vivo, rubro de los 8, email, password) → `POST /api/signup/start` (valida zod+rut+vertical, **rate-limit 5/IP/hora** por `sha256(ip)` sin IP cruda Ley 19.628, guarda `SignupRequest` con password cifrado **AES-256-GCM** + token de un solo uso) → email **Resend** (REST fetch; sin key loguea el link en dev) → click → `GET /api/signup/verify` (consume token **atómico** anti doble-provisión → `POST {PROVISIONING_URL}/admin/v1/tenants` header `X-Provisioning-Key` server-only [SP2] → borra la fila, password ya no se necesita) → redirect a `{APP_URL}/?server&tenant&email` (el login SP3 prellena, con el prefill endurecido). Libs: `rut`/`verticals`/`slug`/`signup-crypto`(AES-GCM+hashIp+token)/`email`/`provisioning`(errores es-CL: TENANT_EXISTS/401/422/503). Migración Prisma `SignupRequest`. CTA "Usar gratis en web" en `/checkout` (landing PR10 pendiente → doc). **Gate verde**: `next build` + vitest **31/31** (12 nuevos: rut mód-11, verticals, slug, crypto round-trip+tamper). **Smoke EN VIVO contra la VM**: `create=201 · dup=409 · badkey=401 · badrut=422 · login=200`. Envs nuevas (Vercel, founder setea): `PROVISIONING_URL/KEY`, `SIGNUP_ENC_KEY`, `RESEND_API_KEY`, `SIGNUP_FROM_EMAIL`, `NEXT_PUBLIC_APP_URL`. **Además (mismo push)**: SP3 login prellena `?server/tenant/email` para el hand-off (commit `433acd8`) + **fix seguridad** de ese prefill (XSS + open-redirect/credential-phishing: escapeAttr + validateServerUrl + no persistir en load, commit `afc6631`, hallazgos del review automático). **Housekeeping**: podados 8 worktrees mergeados (16→8); `stock-multisuc` conservado (⚠ trabajo real sin commit: migr 0033 + dominio stock multisucursal).

- **✅ 2026-07-21 — SP3 SaaS web: ERP corre en BROWSER (lane `feature/saas-web`, PR #330)**: el MISMO frontend del cliente Tauri sirviéndose como PWA contra la API cloud (ADR-0015 P2). **Shim `client/src/web-transport/`** (nuevo): reemplazo drop-in de `@tauri-apps/api/core` para el browser — los **73 comandos** `#[tauri::command]` mapeados 1:1 a su llamada HTTP (`invoke(cmd,args)`→`fetch`), portando de `src-tauri/http.rs`/`state.rs` los MISMOS strings de error en español (envelope `{error:{code,message}}`, `error_message`/`conn_error`/`coded_error` `CODE|message`), sentinels (`CUSTOMERS_MODULE_MISSING`, `SEED_ALREADY_EXISTS`), `Idempotency-Key` por venta, timeouts 30s/5s (`AbortSignal.timeout`), token en memoria+`sessionStorage` (NO localStorage). Agrupado por dominio espejo de `commands/*.rs` (auth/catalog/pos/cash/customers/purchases/dte/misc + registry). **Degradación desktop-only**: `print_ticket`/`open_cash_drawer` (ESC/POS) + updater → `"Disponible en la app de escritorio"` (las vistas ya hacen fallback, no mueren). **Cero cambios en las 18 vistas ni en `src/api/*`** — el shim entra por `resolve.alias` condicional de Vite (`--mode web`) + stub del plugin-updater. **Build web**: `npm run build:web` → `dist-web/` (manifest PWA RutBusiness + iconos + service worker shell-only, datos SIEMPRE a red), CSP con `connect-src`. **GATE client verde**: `tsc` + vitest **741** (10 unit nuevos del shim: mapping, envelope→español, coded, 404→null/sentinel, degradación, token flow) + `build:web` + **e2e 260/0/0**; build desktop intacto. **Bug real cazado y arreglado**: CORS global (`crates/api/src/lib.rs`) no listaba PUT → `set_setting` bloqueado cross-origin (GATE `cargo -p api` verde, 44 suites). **DESPLEGADO VIVO**: **https://rutbusiness-web.vercel.app** (Vercel, estático) contra `https://136.67.83.70.nip.io` (VM `pharma-prod`, server redeployado vía Docker con el fix CORS+PUT). **CORS** habilitado en la VM (`/var/lib/pharma/config/local.toml` `[cors] allowed_origins`, las listas NO se setean por env — gotcha). **Login demo listo**: tenant `demo` (minimarket seedeado) · `dueno@demo.cl` / `Demo2026!` (owner+admin, creado vía CLI en la VM). **Fix UX** (feedback founder): `#nav` del sidebar ahora `overflow-y:auto` (15 secciones se cortaban en viewports bajos/tablet). Commits `93f1032`+`b4dfdaf`+`58d4f26`. Doc `client/README.md` (build/deploy web + CORS). **Nota**: dominio `rutbusiness.cl` aún NO existe (NXDOMAIN) → mientras, nip.io; el default de build (`VITE_DEFAULT_SERVER_URL`) apunta a la IP. Signup multi-tenant self-service = license-server (SP futuro, usa la provisioning key SP2 ya viva); hoy cuentas se crean vía CLI/provisioning.

- **✅ 2026-07-21 — SP2 SaaS web: provisioning API (lane `feature/saas-web`, PR #330)**: `POST /admin/v1/tenants` para que el license-server (signup web) cree tenant + admin. Gated por `PHARMA__PROVISIONING__KEY` (sin env → 404 invisible, on-prem no lo expone; key mala → 401 `PROVISIONING_KEY_INVALID`, comparación constante-tiempo de `X-Provisioning-Key`). Lógica compartida extraída a **`domain::provisioning`** (CLI `tenant-create`/`user-create` refactorizados a la misma) + `validate_rut` mód-11 + `validate_vertical` (catálogo rubro) + RUT guardado como `admin_setting business.rut` (dup slug/RUT → 409 `TENANT_EXISTS`; validación → 422). Tests `crates/api/tests/provisioning.rs`: contrato completo + **gate de aislamiento cross-tenant** (2 tenants vía endpoint: products/orders/settings no se cruzan). GATE workspace verde (fmt+clippy -D+test, 119 suites ok / 0 fallas). Deploy a VM `pharma-prod` con la env nueva: setear `PHARMA__PROVISIONING__KEY` en `/etc/pharma/env` (secreto fuerte, solo license-server lo conoce).
- **✅ 2026-07-21 — SP1 SaaS web: pharma-api VIVO en GCE (lane `feature/saas-web`)**: VM `pharma-prod` (e2-micro free tier, us-west1-b, proyecto `rutbusiness-cloud`, IP estática `136.67.83.70`), Caddy TLS → `https://136.67.83.70.nip.io` (`/health/ready` 200 db:ok + login tenant demo 200 con JWT). Binarios Linux via Docker `rust:1.95` (GATE 0: `cargo check` target Linux verde, **cero cfg-gates que tocar**). Firewall solo 80/443 + SSH por IAP (default-allow-ssh/rdp borrados); snapshots GCE diarios (retención 7) + bucket `gs://rutbusiness-backups`; api escucha `127.0.0.1:8080` detrás de Caddy; JWT secret fuerte por env en la VM. Tenant `demo` (minimarket) seedeado vía CLI en la VM. Entregables: `scripts/deploy-cloud.sh`, `installer/cloud/{pharma-api.service,Caddyfile,env.template}`, runbook `docs/product/saas-web-cloud-ops.md`. **Gotcha nuevo**: CLI usa `AppConfig::load()` estricto pero api usa `load_or_default()` → con env incompleto el api cae SILENCIOSO a defaults (bind 0.0.0.0 + data `./data/surreal`) y api/CLI abren DBs DISTINTAS; el env de la VM debe llevar el set completo (namespace/database/issuer/ttl/otlp). TODOs founder (en runbook): dominio `rutbusiness.cl` (mientras: nip.io), confirmar billing account `01CFAA-B6DE2D-50B576`, scope GCS para cron tar.
- **🔄 2026-06-21 — WAVE 6 en progreso (paxoloop integra incremental)**: "cablear config center end-to-end + adoptar design system + esenciales". **#268 marvin INTEGRADO** (1er PR W6): upload cert + CAF desde UI (reusa dte encrypt-at-rest) + listar respaldos (`crates/api/v1/{dte,backup}.rs` + tests `dte_caf_cert_upload`/`backup`) → habilita que ye cablee las secciones SII/Respaldo del config center. GATE Rust verde (fmt+clippy -D+test, 0 fallas; **lld+sccache activos** — 1er build pobló cache, sin OOM). Conflicto openapi.rs (branches W5 + backup W6, ambos paths) reconciliado keep-both. **Falsos-rojos resueltos**: `CARGO_INCREMENTAL=1` (setx viejo) chocaba con sccache → setx 0 (env override config.toml, ver [[box-build-resource-tuning]]). Pendientes W6: ye (cablear secciones), milton (más acciones agente), paul/bob (adoptar `ui.ts`). **Nuevo plan de horizonte**: [`docs/strategy/business-depth-master-plan.md`](./docs/strategy/business-depth-master-plan.md) — fiado/cuenta-corriente + multi-sucursal operativo + libro compras/IVA (W7+).
- **✅ 2026-06-21 — PRODUCT-WAVE 5 integrada (paxoloop, 5 PRs, incremental)**: directiva founder "config page + UX intuitiva + completo/profesional". Plan: [`docs/strategy/professional-completeness-master-plan.md`](./docs/strategy/professional-completeness-master-plan.md). **#264 ye** — **Centro de Configuración**: `configuracion.ts` → hub multi-sección (side-nav + tarjetas, labels humanos NO keys crudas), `config-center.ts` (8 secciones, búsqueda-en-settings, validación) +50 tests. **#263 paul** — **command palette Ctrl/Cmd+K** (`keymap.ts` fuzzy es-CL + g-chord GOTO + SHORTCUTS) + cheatsheet `?` + overlay, mount 1-línea en shell. **#266 bob** — design system `ui.ts` (.ui-* empty/loading/error/button/card) + 5 vistas adoptan helpers + `fechaHora` canónico + auditoría de-pharma'd. **#267 milton** — backend config: usuarios/roles API + medios de pago + business profile (cierra gap CLI→UI), `crates/api/v1/config.rs`. **#265 marvin** — sucursales/cajas: `crates/domain/branches` + `crates/api/v1/branches.rs` + **mig 0032** (validada contra surrealkv real). **GATE incremental**: client (ye+paul+bob) build+vitest **648** (3-merge incremental, conflictos brand.css `.cfg`/`.cmdk`/`.ui` reconciliados keep-all; split de `config_center.rs` milton↔marvin add/add → `config_branches.rs`); Rust (milton+marvin) fmt+clippy -D + test **112 suites ok / 0 fallas reales**. **NOTA box**: `cargo test --workspace` OOM-mata 1 proceso de test aleatorio por corrida (seed_restaurant, agent_inbox — cada uno PASA aislado; transitorio, NO bug; doc [[integration-gate-concurrency]]). e2e diferido (cubierto por integ-tests que bootean app + mig 0032). **NUEVA base = `feature/erp-parity` (este push). Mig libre = 0033.** ⚠ Manual founder: 1 reboot (pagefile) + tuning build aplicado (jobs=2/debug=0, ver [[box-build-resource-tuning]]). Pendiente founder: LLM opt-in real (ADR-0016) + palancas build sccache/lld.
- **✅ 2026-06-20 — PRODUCT-WAVE 4 integrada (paxoloop, 5 PRs, off `@283107a`)**: "el agente corre el negocio" + datos veraces. **#261 milton** — más acciones del agente (whitelist ampliado: crear_cliente/crear_producto/ajustar_precio sobre catalog/customers + más read intents; contrato `/assist/ask`+`/assist/act` estable). **#262 marvin** — **WAC** (costo promedio ponderado) en recepciones multi-lote → margen veraz en reports/agente. **#259 ye** — agente proactivo (cards dashboard "por vencer/caja sin cerrar") + onboarding del agente. **#258 paul** — POS QoL (hold/recall venta, scan polish, quick-discount). **#260 bob** — export CSV/JSON sin lock-in (ADR-0005) + e2e del flujo de acción del agente. **GATE de record VERDE combinado** (gates SECUENCIALES): Rust workspace fmt+clippy -D+test (0 fallas) · client **550 unit** (vitest `--pool=forks`) + e2e **260/0/0**. NOTA infra: milton+marvin quedaron UNCOMMITTED → rescatados (commit+push paxoloop). **Causa raíz de builds rotos RESUELTA**: box 8c/24GB, cargo default `-j8` → OOM mataba rustc ("could not compile <dep>" aleatorio) → `CARGO_BUILD_JOBS=4` permanente + **sweep de ~90 worktrees stale** (liberó ~370G + presión I/O) — ver [[integration-gate-concurrency]]. Plan próximo en [`docs/strategy/professional-completeness-master-plan.md`](./docs/strategy/professional-completeness-master-plan.md) (config center + UX intuitiva + pulido pro). **NUEVA base = `feature/erp-parity` (este push). Mig libre = 0032.** ⚠ Manual founder: 1 reboot (activa pagefile grande). Pendiente founder: LLM opt-in real (ADR-0016).
...
## 2026-07-25 — V2.1 LOTES/FEFO por sucursal (lane `feat/fefo-por-sucursal`)

- **Qué** — cierra el pendiente explícito que dejó abierto V2 (0041). Hasta hoy el stock por sucursal era un ledger de **cantidades**: la sucursal no podía vender más de lo suyo, pero la elección de **lote** seguía siendo global — vender en el local A podía descontar por FEFO el frasco que está físicamente en el B. El vencimiento que el cajero veía en pantalla no era el envase que tenía en la mano.
- **Migración 0042** (`0042_batch_branch.surql`): `product_batch.branch option<record<branch>>` + índice `pb_tenant_product_branch_expiry (tenant, product, branch, expiry_date)` — el plan FEFO acotado al local resuelve por índice, no por scan (hot path POS <50ms p99). `purchase_order.branch` + `po_tenant_branch_created`. **Sin backfill con `FOR`**: sobre tabla SCHEMAFULL un campo `option` nuevo lee `NONE` en las filas viejas, y `NONE` es exactamente el bucket al que 0041 asignó todo el stock histórico — un instalado hoy queda coherente sin tocar una fila.
- **Invariante que se agrega** al de 0041 (`Σ_sucursal product_branch_stock == product.stock`), para todo producto con lotes: `Σ product_batch.stock WHERE branch = X == product_branch_stock[X]`. Se sostiene porque los tres caminos que mueven lotes estampan la MISMA sucursal en el lote y en el `stock_movement` de la misma transacción.
- **FEFO acotado** (`inventory/repo.rs`): `plan_fefo` y `count_active_batches` toman `branch`. Acotar el **conteo** también importa: si el producto tiene lotes en B pero ninguno en A, vender en A no debe exigir FEFO contra lotes ajenos — cae al camino `product.stock`, que el pre-chequeo por bucket de 0041 ya limitó a lo que A realmente tiene.
- **Venta**: `plan_fefo_optional` recibe la sucursal ya resuelta por la precedencia de 0041 (request → caja abierta del cajero → casa matriz). Sin cambios de contrato HTTP.
- **Recepción**: `branch` se fija al **crear la OC** (el comprador ya sabe para qué local compra) y la recepción la lee de ahí, no del request — así dos recepciones parciales de la misma orden no pueden contradecirse de local. El lote creado y el `stock_movement` llevan esa sucursal. `find_batch` ahora clavea por `(tenant, product, branch, batch_code)`: el mismo código de proveedor en dos locales son **dos lotes físicos distintos**; colapsarlos pondría en un bucket cajas que están en dos lugares.
- **Transferencia — agregado al alcance**: mover sólo la cantidad dejaba al destino con stock que su FEFO no podía consumir (dead-end: stock visible e invendible). `apply_transfer` ahora mueve **lote por lote** en la misma tx (descuenta el de origen, acumula/crea su espejo en el destino), eligiendo por FEFO en el origen — se rota primero lo que vence antes.
- **Decisiones de borde**: `agent_orders` despacha desde casa matriz (mismo bucket que su `stock_movement`, que no lleva `branch`); el borrador de OC del agente entra en casa matriz — elegir local por voz es lane aparte.
- **Tests** — `crates/domain/tests/batch_branches.rs` (8, nuevos): FEFO no cruza aunque el otro local venza antes · orden FEFO dentro del local · rechazo al vender sin lote propio · códigos homónimos en dos locales · recepción aterrizando en un solo local · transferencia habilitando la venta en destino · transferencia manda el que vence primero · compat de negocio mono-local. Invariante 0042 verificado en cada caso.
- **GATE verde** (evidencia real): `cargo check --workspace --all-targets` limpio · `cargo fmt --all -- --check` limpio · **`cargo test -p domain --test batch_branches` 8 passed / 0 failed** · **`cargo test -p api` 307 passed / 0 failed / 2 ignored**, incluido el guardián **`e2e_concurrency_fefo` 4/4** (60 ventas paralelas sobre un lote, sin sobre-venta) — acotar FEFO por sucursal NO reabre BUG-003/004 porque el filtro entra dentro del mismo `tenant_stock_lock`, sin agregar camino de escritura.
- **SMOKE EN VIVO 2 sucursales** (`scripts/qa/fefo-sucursales-live-qa.sh`, nuevo; server real + SurrealKv + migraciones al boot, `migrate complete total=42 applied=42`): FEFO no cruzó (L-CASA 20→15 con L-CENTRO venciendo antes e intacto en 20) · B no vende lo de casa matriz (422 "stock insuficiente") · recepción en B subió sólo B 20→50 con casa matriz sin cambios · el lote nació en B y no existe en casa matriz · FEFO dentro de B agotó L-CENTRO (10d) antes de L-RECIBIDO (200d) · invariante 0042 cuadrado en ambos locales tras cada movimiento. **TODO VERDE.**
- **Ripple** — 30 literales `NewBatch`/`NewPurchaseOrder` + 3 llamadas a `plan_fefo*` en tests existentes, todos a `None` = casa matriz (donde esos tests ya tenían sus lotes; no cambia lo que asertan).
- **Fuera de alcance, explícito**: alertas de **vencimiento por local** (`jobs/near_expiry.rs` sigue siendo global) y el selector de sucursal para lotes en el cliente Tauri. **Migración libre = 0043.**
