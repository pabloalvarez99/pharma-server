...
- **🅿️ PARKED — BUG-003/004 (POS sale concurrency, alta prioridad backend)**: el hot path de venta (`domain::sales::service::post_sale`/`apply_sale`) NO es concurrency-safe — bajo contención SurrealKv aborta las txns perdedoras con un conflicto MVCC *reintentable* y el path NO reintenta/serializa → ~59/60 ventas concurrentes fallan con DB_ERROR 500 (en vez de éxito o `INSUFFICIENT_STOCK` limpio) y `product.stock` queda corrupto. Tests ya escritos + `#[ignore]`d en `crates/api/tests/e2e_concurrency_fefo.rs`. **Acción-dueño**: lane dedicada con dueño humano que añada retry-on-conflict o serialización (patrón `crates/dte/src/caf.rs::ASSIGN_LOCK`) en **`crates/domain::sales::service`** (fuera del scope de la lane Lane A backend-license) — desbloquea 3 tests `#[ignore]` + arregla el bug más importante del producto (POS). Alto riesgo de concurrencia: requiere verificación, no apto para trabajo desatendido.
