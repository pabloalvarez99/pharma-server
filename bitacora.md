...
- **⛔ PARKED (forcing function, rule #9) — deploy MSI autónomo por CI**: el workflow `release-publisher.yml` (build `cargo wix` → sign self-sign cert → publish al mirror) está **escrito y mergeado** (PR #87) pero **NO puede correr hasta que el FUNDADOR agregue 3 GitHub secrets** al repo `pabloalvarez99/pharma-server`: `PILOT_PFX_B64` (base64 de `installer/sign/pilot.pfx`), `PHARMA_CERT_PASSWORD` (ya en User env local, 48 chars), `MIRROR_RELEASE_TOKEN` (PAT con contents:write en `pharma-server-releases`). **Acción-dueño**: fundador corre los 3 `gh secret set` (runbook en sesión 2026-05-30) → desbloquea `gh workflow run release-publisher.yml --ref feature/erp-parity` 100% hands-off. Workaround actual sigue siendo `gh release create` directo con build+sign local (ya probado, MSI 0.1.27 publicado).
- **Cliente Tauri — ERP parity COMPLETA (2026-05-30)**: las vistas operables son POS (cliente+fidelidad, boleta, quick-cash+vuelto, scan), Inventario+lotes/vencimientos, Caja (apertura/arqueo/cierre), Clientes (CRUD), Compras (proveedores + OC create multilínea + recepción de mercadería), Gastos, Reportes (márgenes Pro-gated + rotación), Recetas+Libro de controlados (Ley 20.000, export CSV), Dashboard. **0 comandos Tauri huérfanos** (todos los `#[tauri::command]` definidos en `lib.rs` están en `invoke_handler!`). Últimas lanes mergeadas a `feature/erp-parity`: Recetas (PR #100 `17f43cd`), Compras OC create/receive (PR #101 `e585b82`). Pile limpio: 0 PRs abiertos, 0 worktrees. **Gotcha permanente**: `client/src-tauri` está EXCLUIDO del workspace cargo → CI clippy no lo chequea; GATE client = `cd client && npm run build` (tsc+vite). Structs anidados en args Tauri = snake_case (serde, sin rename); money STRING.
...
- **Versión**: `0.1.27` (workspace `Cargo.toml`) — bump fix static-CRT (servicio arranca en Windows limpio). **MSI 0.1.27 PUBLICADO** al mirror público.
