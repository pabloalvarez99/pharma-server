...
## 2026-06-13 — feat(cli): `pharma seed-demo` multi-rubro + visión RutAgentIA consolidada

- **Qué**: nuevo comando `pharma seed-demo --tenant <slug> [--vertical pharmacy|minimarket] [--force]` (`crates/cli/src/seed_cmd.rs` + wiring en `main.rs`). Puebla un tenant con un negocio chileno demo para poder PROBAR la app contra datos creíbles (no pantallas vacías). Idempotente (marca DEMO: `external_id=DEMO-*`, `reason=seed_demo`, `customer.email=demo.*@demo.rutagentia.local`); `--force` borra SOLO lo DEMO y re-siembra (también para cambiar de rubro).
- **Multi-rubro a propósito** (visión RutAgentIA, [`docs/strategy/rutagentia-vision.md`](./docs/strategy/rutagentia-vision.md) nuevo): dos packs sobre el MISMO schema — `pharmacy` (16 prod: OTC + recetados + 1 controlado Ley 20.000 + lotes por vencer) y `minimarket` (8 abarrotes, sin campos clínicos). Demuestra que el core ES agnóstico de rubro; farmacia = beachhead, no límite. Más rubros = agregar un pack; el core no cambia.
- **Honra invariantes**: por producto crea product(stock=N) + product_batch(stock=N) + stock_movement(delta=+N) → `product.stock == Σ batch.stock == Σ movimientos`.
- **Menos invasivo**: CLI local (corre como `pharma migrate`), reusa el DB handle directo; sin endpoint, sin migración, sin tocar el cliente.
- **Verificado E2E**: migrate(23) → tenant → seed minimarket(8) → re-seed no-op → `--force` pharmacy(16) → vertical inválido = error claro. fmt + clippy `-D warnings` verde.
- **Archivos**: `crates/cli/src/seed_cmd.rs` (nuevo), `crates/cli/src/main.rs` (subcommand), `docs/strategy/rutagentia-vision.md` (nuevo).
