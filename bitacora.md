...
- **📌 PENDIENTE EXPLÍCITO — V2.1 LOTES FÍSICOS POR SUCURSAL (`product_batch.branch`)**: la lane V2 de abajo dejó el stock por sucursal como **ledger de CANTIDADES**: la sucursal no puede vender ni transferir más de lo que tiene, pero **la elección de lote por vencimiento (FEFO) sigue siendo GLOBAL** — el local A puede consumir por FEFO un lote que físicamente está en el B. Consecuencias acotadas y conocidas: la trazabilidad de lote por local y las alertas de vencimiento por local todavía no son fieles. **Alcance del fix (lane siguiente, NO empezada)**: campo `product_batch.branch option<record<branch>>` (migración nueva, backfill a casa matriz) + `inventory::service::plan_fefo_optional` acotado a la sucursal de la venta + la transferencia moviendo cantidad DE LOTE (FEFO en el origen, upsert de la fila de lote en el destino) + recepción de compras asignando sucursal + alertas de vencimiento por local. **Dueño**: siguiente lane de backend. **Por qué se separó**: el blast radius toca recepción de compras, `agent_orders` y compliance de vencimientos; meterlo en la misma lane dejaba dos módulos a medias en vez de uno completo (decisión del fundador, 2026-07-25).
