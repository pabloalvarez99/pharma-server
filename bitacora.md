...
- **Fase 9.1 DTEs SII (Native Rust, ADR-0011) — avance 2026-05-31**: hechas subtasks 9.1.a (XML boleta 39), b (TED RSA-SHA1), **b.2 (firma XML-DSig del `<Documento>` con cert empresa — PR #105 merged)**, c (CAF folio atómico), d/e (envío + polling SII), f-parcial (cancel/resend), h (X/Z), i (cert encrypt-at-rest), j (gating tier). **Pendientes**: 9.1.b.3 (PFX/PKCS#12 parse nativo — hoy `KeyMaterial::from_pem`), 9.1.b.4 (C14N 1.0 full gated por sandbox SII), 9.1.f (render tipos 33/56/61/52), 9.1.g (libro ventas mensual), 9.1.l-m (integration sandbox + docs cliente). **El crate `dte` YA está cableado a `/api/v1/dte/*` (2026-06-09)**: emit boleta desde orden POS + list/get/export-XML + caf-status + send SII tier-gated (402 Free) + poll + cancel — ver entrada 2026-06-09. CLI 9.1.k existente (`pharma dte|caf|cert`). Falta para flujo cliente completo: vista Tauri de boletas + setting UI de `dte.emisor`.
